...
# -------------------------------------------------- #
    ...
    frac_accepted = N_accepted/N_proposals

    print(
        f"Accepted: {N_accepted}, Rejected: {N_rejected}, "
        f"Fraction: {frac_accepted:.2f}"
    )
    ...
    print(
        ...
        f"samples on an L={L} lattice: {time.time() - start_time:.2g} seconds"
    )
    print(f"Integrated autocorrelation time: {integrated_autocorr:.2f}")


    return frac_accepted, integrated_autocorr, autocorr_np

...
class LegacyAffineLayer(nn.Module):
    r"""Class implementing 'legacy' version of affine coupling layers.

    These differ from :py:class:`anvil.layers.AffineLayer` in that the
    's' and 't' parameters are generated by *different* networks, rather
    than by different channels of the same network.

    Only fully connected networks are supported.
    """
    ...
    def transform(
        ...
        """Transform the active partition."""
        v_for_net = v_passive / torch.sqrt(v_passive.var() + 1e-6)
        s = self.s_func(s_net(v_for_net)).squeeze()
        t = t_net(v_for_net).squeeze()
    ...
    def forward(self, v_in: torch.Tensor, log_density: torch.Tensor):
        """Forward pass of the legacy affine layer."""
        v_r = v_in[:, self.mask]
        v_b = v_in[:, ~self.mask]

        v_r, log_density = self.transform(v_r, v_b, log_density, self.s1, self.t1)
        v_b, log_density = self.transform(v_b, v_r, log_density, self.s2, self.t2)
...
class LegacyEquivariantSplineLayer(nn.Module):
    r"""Class implementing 'legacy' version of equivariant spline coupling layers.

    The difference with :py:class:`anvil.layers.RationalQuadraticSplineLayer` is
    that we enforce Z_2 equivariance by treating inputs with positive and negative
    magnetisation slightly differently.
    ...
    **HEALTH WARNING:** Unfortunately, this results in transformations that are not
    ...
    def __init__(
        ...
        *,
        ...
        log.warning(
            "Use of the legacy equivariant spline layer is not recommended since correct density estimation cannot be guaranteed!"
        )
    ...
    def transform(
        ...
        log_density: torch.Tensor,
        net: (DenseNeuralNetwork, ConvolutionalNeuralNetwork),
        idx_negative_mag: torch.BoolTensor,
    ):
        """Transform the active partition."""
        # Dims (n_batch, n_lattice). Easier to read v_in v.s. v_active
        v_in = torch.flatten(v_active, start_dim=1)
        ...
        # !EQUIVARIANCE!
        v_for_net[idx_negative_mag] = -v_for_net[idx_negative_mag]
        ...
        # !EQUIVARIANCE!
        # Have to clone for inplace op
        h_net = h_net.clone()
        w_net = w_net.clone()
        d_net = d_net.clone()
        # dims=-1, not 2, in case of idx_negative_mag = []
        h_net[idx_negative_mag] = torch.flip(h_net[idx_negative_mag], dims=(-1,))
        w_net[idx_negative_mag] = torch.flip(w_net[idx_negative_mag], dims=(-1,))
        d_net[idx_negative_mag] = torch.flip(d_net[idx_negative_mag], dims=(-1,))
    ...
    def forward(self, v_in: torch.Tensor, log_density: torch.Tensor):
        ...
        # !EQUIVARIANCE!
        # NOTE: there are two transformations, but we only split the inputs into +/-
        # magnetisation before the first transf.
        idx_negative_mag = (v_in.sum(dim=1).sign() < 0).nonzero().squeeze()

        v_r = v_in[:, self.mask]
        v_b = v_in[:, ~self.mask]

        v_r, log_density = self.transform(
            v_r, v_b, log_density, self.net1, idx_negative_mag
        )
        v_b, log_density = self.transform(
            v_b, v_r, log_density, self.net2, idx_negative_mag
        )

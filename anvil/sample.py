...
def gen_candidates(model, base, target, num):
    ...
    phi, model_log_density = model(z, 0, negative_mag)
...
def metropolis_hastings(
    loaded_model_from_training,
    base_from_training,
    target_from_training,
    sample_size,
    thermalization,
    sample_interval,
):
    ...
    phi, log_ratio = gen_candidates(
        loaded_model_from_training, base_from_training, target_from_training, num=1
    )
    current = phi[0]
    ...
    configs_out = torch.empty(
        (sample_size, base_from_training.size_out), dtype=torch.float32
    )
    history = []

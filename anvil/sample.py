...
def produce_samples(
    loaded_model, action, length, state_i=None, log_ratio_i=None, first=False
):
    r"""
    ...
    state_i: torch.Tensor
        the final phi state of the chain, for continuity between batches
    log_ratio_i: float
        the ratio associated with state_i
    """
    with torch.no_grad():  # don't track gradients
        z = torch.randn((length, loaded_model.size_in))  # random z configurations
...
def sample(loaded_model, action, target_length: int, tskip_guess=-1) -> torch.Tensor:
    ...
    tskip_guess: int
        an initial guess for the subsampling interval required to produce
        decorrelated configurations. By default, this is computed automatically
        using the first batch of configurations.
    ...
    batch_chain, batch_history, state_i, log_ratio_i = produce_samples(
    ...
    if tskip_guess < 1:  # i.e. no guess has been attempted
        ...
        print(
            f"Guess for tskip: {tskip_guess}, based on {batch_length} configurations."
        )
    ...
    # Add first batch
    full_chain[:batch_length, :] = batch_chain
    ...
    for batch in range(1, Nbatches):
        ...
        full_chain[start : start + batch_length, :] = batch_chain

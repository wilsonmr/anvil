...
def print_format(val, err):
    ...
    prec = floor(log10(abs(float(err))))
    if prec < 0:
        val_str = np.format_float_positional(val, -prec, unique=False, fractional=True, pad_right=1)
        err_str = np.format_float_positional(err, -prec, fractional=True, pad_left=1)
    else:
        int_prec = ceil(log10(abs(float(val))))
        val_str = np.format_float_positional(val, int_prec-prec, fractional=False, pad_right=1).strip(".")
        err_str = np.format_float_positional(err, 1, fractional=False, pad_left=1).strip(".")
    return val_str, err_str


@table
def ising_observables_table(ising_energy, susceptibility, bootstrap_std, training_output):
    ...
    res = [[IE, IE_std],
           [S, S_std]]
    df = pd.DataFrame(
        res, columns=["Mean", "Standard deviation"], index=["Ising energy", "susceptibility"]
    )
    return df

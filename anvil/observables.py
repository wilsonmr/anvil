...
def fit_zero_momentum_correlator(zero_momentum_correlator, geometry_from_training):
...
def optimal_window(integrated, mult=2.0, eps=1e-6):
    ...
    # Infer configs size. Assumes correlation mode was 'same'
...
def two_point_correlator(
    configs, geometry_from_training, bootstrap_sample_size, bootstrap_seed
):
    """Helper function which executes multiprocessing function to calculate the
    bootstrapped two point correlation function."""

    correlator = np.empty((geometry_from_training.volume, bootstrap_sample_size))
    for i, shift in enumerate(geometry_from_training.two_point_iterator()):
    ...
    return correlator.reshape(
        (geometry_from_training.length, geometry_from_training.length, -1)
    )
...
def inverse_pole_mass(effective_pole_mass, geometry_from_training):
    T = geometry_from_training.length
    t0 = T // 4
    window = slice(t0, T - t0 + 1)

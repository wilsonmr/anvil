...
class ConfigParser:
    ...
    def check_key(spec: dict, key: str, typing, allow_none=False):
        ...
        if not isinstance(spec[key], typing):
            raise ConfigError(f"{key} should be of type {typing}, not {type(spec[key])}")
        return spec[key]
    ...
    def parse_model(self, model_spec):
        ...
        return dict(n_affine=n_affine, hid_shape=network['hidden_nodes'], checkpoint=checkpoint)
    ...
    def parse_sample(self, sample_spec):
        ...
        return dict(chain_length=chain_length, thermalisation=therm, autocorr=autocorr, observables=observables)

    def parse_observables(self, obs_list):
        for obs in obs_list:
            if not hasattr(Observables, obs):
                raise ConfigError(f"The following observable hasn't been implemented: {obs}")
        return obs_list
    ...
    def resolve(self):
        ...
        if 'sample' in self.config.keys():
            if mod_dict['checkpoint'] is None and 'train' not in self.config.keys():
                raise ConfigError("If sampling, either specify to `train`, or specify checkpoint in `model`")
